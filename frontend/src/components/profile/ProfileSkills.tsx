...
import { Skill, SkillCategory, SkillProficiency, SKILL_CATEGORIES, PROFICIENCY_LEVELS, COMMON_SKILLS } from '../../types/skills';
...
export default function ProfileSkills({ userId }: Props) {
  ...
  const [form, setForm] = useState<{ name: string; category: SkillCategory; proficiency: SkillProficiency }>({
    name: '', category: 'Technical', proficiency: 'Beginner'
  });
  ...
  const categorySummaries = useMemo(() => {
    ...
    filtered.forEach(s => { res[s.category][s.proficiency] += 1; });
    return res;
  }, [filtered]);
  ...
  async function onAdd(e: React.FormEvent) {
    ...
    // Duplicate prevention (frontend guard)
  ...
  function exportCSV() {
    ...
    a.href = url; a.download = 'skills.csv'; a.click();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add form */}
      <form onSubmit={onAdd} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 rounded-xl border">
        <div className="col-span-1 md:col-span-2">
          ...
          <datalist id="skill-suggestions">
            {COMMON_SKILLS[form.category].map(s => <option key={s} value={s} />)}
          </datalist>
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-xs text-gray-500">
                    {Object.entries(summary).map(([level, count]) => (
                      <span key={level} className="mr-3">{level}: <strong>{count}</strong></span>
                    ))}
                  </div>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-5 max-w-sm w-full">
            ...
            <div className="flex justify-end gap-2">
              <button className="px-3 py-1 rounded border" onClick={() => setConfirmDelete(null)}>Cancel</button>
              <button className="px-3 py-1 rounded bg-red-600 text-white" onClick={onDeleteConfirmed}>Remove</button>
            </div>
          </div>
        </div>
      )}
